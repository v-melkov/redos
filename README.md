...
КриптоПро видит контейнеры только в определённом каталоге  

Можно просто скопировать контейнеры с флешки в каталог, указанный ниже, но это нифига __не безопасно!__  
Можно сделать софтлинк со смонтированной флешки в каталог, но это не универсально  
...
- если есть __токен__ с контейнерами, стандартными средствами криптопро копируй их на созданный диск (криптопро вроде их видит без танцев с бубнами) - назначение будет HDIMAGE.....(надо проверить - токенами на линуксе не пользовался)  
...
VeraCrypt есть для всех ОС - рекомендую под виндой также ей пользоваться (там можно поставить авторазмонтирование - вообще красота)   
